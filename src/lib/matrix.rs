...
impl Matrix {
    ...
    pub fn randoms(rows: usize, cols:usize) -> Matrix {
    ...
    pub fn multiply(&mut self, other: &Matrix) -> Matrix {
        if self.cols != other.rows {
            panic!("Multiply by matrix of incorrect dimensions.")
        }
    ...
    pub fn add(&mut self, other: &Matrix) -> Matrix {
        if self.rows != other.rows || self.cols != other.cols {
            panic!("Add by matrix of incorrect dimensions.")
        }
    ...
    pub fn subtract(&mut self, other: &Matrix) -> Matrix {
        if self.rows != other.rows || self.cols != other.cols {
            panic!("Subtract by matrix of incorrect dimensions.")
        }
        let mut res = Matrix::zeros(self.rows, self.cols);
    ...
    pub fn dot_multiply(&mut self, other: &Matrix) -> Matrix {
        if self.rows != other.rows || self.cols != other.cols {
            panic!("Dot Multiply by matrix of incorrect dimensions.")
        }
        let mut res = Matrix::zeros(self.rows, self.cols);
